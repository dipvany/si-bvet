...
export default function Pengaduan() {
  ...
  const validate = () => {
    ...
    if (!form.nik)               return "Nomor Induk Kependudukan (NIK) wajib diisi.";
    if (!form.date_of_complaint) return "Tanggal melapor wajib diisi.";
    if (!form.subjects)          return "Uraian pelayanan yang tidak sesuai standar wajib diisi.";
    if (!form.description)       return "Saran dan gagasan wajib diisi.";
    if (!attachment)             return "Lampiran bukti pendukung wajib diunggah.";
  ...
  const resetForm = () => {
    setSuccess(false);
    setForm({ fullname:"", address:"", nik:"", date_of_complaint:"", subjects:"", description:"" });
    setAttachment(null);
    setAgreed(false);
    setError("");
  };
