...
​
  ...
  const handleUploadTemplate = async (file) => {
    if (!file) return;
    setUploadingTpl(true); setTplSuccess(""); setTplError("");
    try {
      const fd = new FormData();
      fd.append("file", file);
      const res  = await apiFetch("/superadmin/submissions/samples/template", {
        method: "POST",
        body: fd,
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error ?? json.message ?? "Gagal mengupload template.");
      setTplSuccess("Template berhasil diupload. Pelanggan dapat mengunduh template terbaru di Step 2 pengajuan.");
      setShowTplModal(false);
    } catch (err) {
      setTplError(err.message ?? "Gagal mengupload template.");
    } finally {
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <button
                ...
                className="flex items-center gap-1.5 text-xs font-semibold text-white
                  bg-[#233B6E] hover:bg-[#1a2d56] px-3 py-2 rounded-lg transition-colors shadow-sm">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"
                  strokeLinecap="round" strokeLinejoin="round" className="w-3.5 h-3.5">
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {loading ? (
                  ...
                  <tr><td colSpan={selectMode ? 9 : 8} className="px-4 py-14 text-center text-gray-400 text-sm">
                    {search || filterStatus !== "all" ? "Tidak ada hasil yang cocok." : "Belum ada pengajuan masuk."}
                  </td></tr>
                ) : filtered.map((s, i) => (
                  <tr key={s.id}
                    className="hover:bg-[#F6F7FB] transition-colors cursor-pointer"
                    onClick={() => selectMode ? toggleSelectOne(s.id) : navigate(`detail/${s.id}`, { state: { submission: s } })}>
                    {selectMode && (
                    ...
                    <td className="px-4 py-3 text-gray-500 text-xs">#{s.user_id}</td>
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className="inline-flex items-center justify-center
                        w-7 h-7 rounded-full bg-[#EEF0F8] text-[#233B6E] text-xs font-bold">
                        {s.samples_count ?? 0}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                      <button
                        onClick={() => navigate(`detail/${s.id}`, { state: { submission: s } })}
...
​
      ...
      {tplSuccess && (
        <div className="fixed bottom-5 right-5 z-50 max-w-sm bg-green-600 text-white
          text-sm font-semibold px-5 py-3 rounded-xl shadow-xl flex items-start gap-3">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"
            strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4 flex-shrink-0 mt-0.5">
            <polyline points="20 6 9 17 4 12"/>
          </svg>
          <span className="flex-1">{tplSuccess}</span>
          <button onClick={() => setTplSuccess("")}
...
​
      ...
      {showTplModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/50" onClick={() => setShowTplModal(false)} />
...
​
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h2 className="font-bold text-[#233B6E] text-base">Import Template Excel</h2>
                  <p className="text-xs text-gray-400 mt-0.5">
                    Upload file template yang akan diunduh customer di Step 2 pengajuan massal.
                  </p>
                </div>
...
​
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-xl px-4 py-3
                ...
                <ol className="list-decimal list-inside space-y-0.5 text-blue-600">
                  <li>Upload template Excel di sini (superadmin)</li>
                  <li>Customer buka Pengajuan Uji Sampel → Step 2</li>
                  <li>Customer centang "pengajuan massal" → klik <strong>Unduh</strong></li>
...
​
              ...
              <div
                className="border-2 border-dashed border-gray-200 rounded-xl p-6
                  text-center cursor-pointer hover:border-[#233B6E]/50 hover:bg-[#F6F7FB]
                  transition-colors"
              ...
              >
                ...
                <input ref={templateRef} type="file" accept=".xlsx,.xls,.csv"
                  className="hidden"
                  onChange={e => {
