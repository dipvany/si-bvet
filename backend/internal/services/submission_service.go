...
func ParseSamplesFromTemplateExcel(file io.Reader) ([]dto.SampleInput, error) {
	...
	for rowIdx := headerRowIdx + 1; rowIdx < len(rows); rowIdx++ {
		...
		sample := dto.SampleInput{
			...
			LocationType:   getCellValueByHeader(row, headerMap, []string{"locationtype", "location type", "jenis lokasi"}),
			LocationSmpl: getCellValueByHeader(row, headerMap, []string{"locationsmpl", "lokasi sampel"}),
...
func resolveSampleTemplateHeaderMap(rows [][]string) (map[string]int, int, error) {
	headerSpecs := []headerSpec{
		...
		{canonical: "locationtype", aliases: []string{"locationtype", "location type", "jenis lokasi"}},
		{canonical: "locationsmpl", aliases: []string{"locationsmpl", "lokasi sampel"}},
